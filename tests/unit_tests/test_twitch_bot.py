...
class TestTwitchBot(unittest.TestCase):
    ...
    def setUp(cls, mock_open, mock_irc_bot, mock_user_db) -> None:
        cls.bot = TwitchBot()
        cls.bot.initial_channel_ids = ['test_id_1', 'test_id_2', 'test_id_3']
        cls.bot._prefix = '!'
        cls.loop = asyncio.get_event_loop()

    @patch('builtins.open')
    @patch('bots.twitch_bot.os')
    def test_inform_user_on_updates_sends_irc_message_to_osu_username(self, mock_open, mock_os):
    ...
    @patch('builtins.open')
    @patch('bots.twitch_bot.os')
    def test_inform_user_on_updates_calls_db_set_channel_updated_with_twitch_username(self, mock_open, mock_os):
    ...
    def test_handle_request_calls_check_message_contains_beatmap_link(self):
        ...
        self.loop.run_until_complete(self.bot.handle_request(msg))
        self.bot._check_message_contains_beatmap_link.assert_called_once()

    def test_handle_request_calls_check_user_cooldown(self):
    ...
    def test_handle_request_calls_check_request_criteria(self):
    ...
    def test_handle_request_calls_send_twitch_message_when_echo_enabled(self):
        check_request_criteria_return_value = asyncio.Future()
        check_request_criteria_return_value.set_result(None)
    ...
    def test_handle_request_adds_request_to_statistics_db(self):
        check_request_criteria_return_value = asyncio.Future()
        check_request_criteria_return_value.set_result(None)
        self.bot._check_request_criteria = MagicMock(return_value=check_request_criteria_return_value)
        self.bot._check_message_contains_beatmap_link = MagicMock(return_value=(0, 'test_beatmap_id'))
    ...
    def test__check_user_cooldown_calls__prune_cooldowns(self):
        author = MagicMock()
    ...
    def test__check_user_cooldown_adds_user_to_dict_when_not_on_cooldown(self):
        author = MagicMock()
        author.id = 'test_id'
    ...
    def test__check_user_cooldown_raises_assertion_error_if_a_user_is_on_cooldown(self):
        author = MagicMock()
        author.id = 'test_id'
    ...
    def test__check_user_cooldown_updates_user_cooldown_if_not_on_cooldown(self):
        author = MagicMock()
        author.id = 'test_id'
    ...
    def test__prune_cooldowns_removes_user_not_on_cooldown(self):
        user_last_request_time = datetime.datetime.now() - datetime.timedelta(seconds=self.bot.PER_REQUEST_COOLDOWN)
        self.bot.user_last_request = {
            'test_id': user_last_request_time}
    ...
    def test__prune_cooldowns_keeps_user_on_cooldown_in_list(self):
    ...
    def test__send_twitch_message_sends_message_to_context(self):
        msg = MagicMock()
        ...
        self.loop.run_until_complete(self.bot._send_twitch_message(msg, beatmap_info))
        msg.channel.send.assert_called_once()

    def test__send_irc_message_calls_irc_bot_send_message(self):
        msg = MagicMock()
        beatmap_info = MagicMock()
        mods = MagicMock()
        ...
        self.loop.run_until_complete(self.bot._send_irc_message(msg, beatmap_info, mods))
        self.bot.irc_bot.send_message.assert_called_once()

    def test__send_irc_message_calls__prepare_irc_message(self):
        msg = MagicMock()
        beatmap_info = MagicMock()
        mods = MagicMock()
    ...
    def test_event_ready_calls_fetch_users(self):
    ...
    def test_event_ready_calls_join_channels(self):
        join_channels_return_value = asyncio.Future()
        join_channels_return_value.set_result(None)
        self.bot.join_channels = MagicMock(return_value=join_channels_return_value)
        self.bot.load_module = MagicMock()
